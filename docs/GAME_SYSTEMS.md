...
**Roster catalog (pack JSON)** = แม่แบบตอนเริ่มอาชีพ · **Live DB** = `GameSave.players` ใน IndexedDB (ตามย้าย/สถานะ) ดูที่ `/database` · คู่มือ [DATA_IMPORT.md](./DATA_IMPORT.md#นักเตะกับ-db-ตอนนี้ทำยังไง) · Data Hub (`/data`) = วิเคราะห์แมตช์

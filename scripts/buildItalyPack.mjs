/**
 * Italy Serie A pipeline.
 * Usage: node scripts/buildItalyPack.mjs --download-clubs | --resolve | --fetch-fminside | --all
 */
import fs from 'fs'
...
const FMTU_DIR = path.join(ROOT, 'scripts/_fm26_dumps/fmtu_ita')
const FMIN_DIR = path.join(ROOT, 'scripts/_fm26_dumps/fminside')
const IDS_OUT = path.join(ROOT, 'scripts/_fm26_dumps/ita_fm_ids.json')
const PLAYERS_OUT = path.join(ROOT, 'src/data/world/playersIta.json')

/** FM unique club IDs — Serie A 25/26 (20 teams) */
const CLUBS = {
  int: { id: 1135, name: 'Inter Milan', rep: 90 },
  mil: { id: 1099, name: 'AC Milan', rep: 87 },
  juv: { id: 1139, name: 'Juventus', rep: 86 },
  nap: { id: 1150, name: 'Napoli', rep: 85 },
  ata: { id: 1106, name: 'Atalanta', rep: 83 },
  rom: { id: 1100, name: 'AS Roma', rep: 82 },
  laz: { id: 1140, name: 'Lazio', rep: 80 },
  fio: { id: 1129, name: 'Fiorentina', rep: 78 },
  bol: { id: 1111, name: 'Bologna', rep: 77 },
  tor: { id: 1174, name: 'Torino', rep: 74 },
  udi: { id: 1178, name: 'Udinese', rep: 71 },
  gen: { id: 1132, name: 'Genoa', rep: 70 },
  cag: { id: 1114, name: 'Cagliari', rep: 68 },
  lec: { id: 1141, name: 'Lecce', rep: 67 },
  ver: { id: 2201, name: 'Hellas Verona', rep: 66 },
  par: { id: 1156, name: 'Parma', rep: 68 },
  com: { id: 1123, name: 'Como', rep: 65 },
  sas: { id: 3800256, name: 'Sassuolo', rep: 69 },
  pis: { id: 2215, name: 'Pisa', rep: 64 },
  cre: { id: 1125, name: 'Cremonese', rep: 63 },
}
...
function resolveAndBuildPlayers() {
  ...
  fs.writeFileSync(
    IDS_OUT,
    JSON.stringify({ source: 'fmtransferupdate Serie A', matched: Object.keys(byName).length, byName }, null, 2),
  )
  fs.writeFileSync(
    PLAYERS_OUT,
    JSON.stringify(
...
async function fetchFmInside() {
  ...
  let skip = 0
  let fail = 0
  const tmp = path.join(FMIN_DIR, '_tmp_ita.html')

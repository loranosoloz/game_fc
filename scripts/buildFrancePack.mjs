/**
 * France Ligue 1 pipeline.
 * Usage: node scripts/buildFrancePack.mjs --download-clubs | --resolve | --fetch-fminside | --all
 */
import fs from 'fs'
...
const FMTU_DIR = path.join(ROOT, 'scripts/_fm26_dumps/fmtu_fra')
const FMIN_DIR = path.join(ROOT, 'scripts/_fm26_dumps/fminside')
const IDS_OUT = path.join(ROOT, 'scripts/_fm26_dumps/fra_fm_ids.json')
const PLAYERS_OUT = path.join(ROOT, 'src/data/world/playersFra.json')

/** FM unique club IDs — Ligue 1 25/26 (18 teams) */
const CLUBS = {
  psg: { id: 868, name: 'Paris Saint-Germain', rep: 91 },
  om: { id: 866, name: 'Olympique de Marseille', rep: 80 },
  ol: { id: 865, name: 'Olympique Lyonnais', rep: 78 },
  asm: { id: 826, name: 'AS Monaco', rep: 81 },
  lil: { id: 858, name: 'Lille', rep: 79 },
  ren: { id: 884, name: 'Stade Rennais', rep: 76 },
  nic: { id: 862, name: 'OGC Nice', rep: 75 },
  rcl: { id: 871, name: 'RC Lens', rep: 76 },
  nte: { id: 846, name: 'Nantes', rep: 70 },
  str: { id: 872, name: 'Strasbourg', rep: 72 },
  tou: { id: 886, name: 'Toulouse', rep: 70 },
  sbr: { id: 2061, name: 'Brest', rep: 73 },
  aux: { id: 824, name: 'Auxerre', rep: 66 },
  ang: { id: 875, name: 'Angers', rep: 65 },
  hac: { id: 856, name: 'Le Havre', rep: 66 },
  lor: { id: 2005, name: 'Lorient', rep: 64 },
  pfc: { id: 867, name: 'Paris FC', rep: 68 },
  met: { id: 844, name: 'Metz', rep: 63 },
}
...
function resolveAndBuildPlayers() {
  ...
  fs.writeFileSync(
    IDS_OUT,
    JSON.stringify({ source: 'fmtransferupdate Ligue 1', matched: Object.keys(byName).length, byName }, null, 2),
  )
  fs.writeFileSync(
    PLAYERS_OUT,
    JSON.stringify(
...
async function fetchFmInside() {
  ...
  let skip = 0
  let fail = 0
  const tmp = path.join(FMIN_DIR, '_tmp_fra.html')

 ...
 *   node scripts/buildThaiPack.mjs --all
 */
import fs from 'fs'
...
const FMTU_DIR = path.join(ROOT, 'scripts/_fm26_dumps/fmtu_tha')
const FMIN_DIR = path.join(ROOT, 'scripts/_fm26_dumps/fminside')
const IDS_OUT = path.join(ROOT, 'scripts/_fm26_dumps/tha_fm_ids.json')
const PLAYERS_OUT = path.join(ROOT, 'src/data/world/playersTha.json')

/** FM unique club IDs — Thai League 1 25/26 (16 teams) */
const CLUBS = {
  bru: { id: 23177015, name: 'Buriram United', rep: 82 },
  ptc: { id: 23295496, name: 'Port FC', rep: 79 },
  rcb: { id: 23038393, name: 'Ratchaburi Mitr Phol', rep: 78 },
  bgp: { id: 23485682, name: 'BG Pathum United', rep: 76 },
  bku: { id: 23078011, name: 'Bangkok United', rep: 77 },
  prc: { id: 23268087, name: 'PT Prachuap', rep: 71 },
  chi: { id: 23077991, name: 'Chiangrai United', rep: 69 },
  chu: { id: 5637855, name: 'Chonburi FC', rep: 70 },
  ryg: { id: 5625051, name: 'Rayong FC', rep: 64 },
  aya: { id: 23348779, name: 'Ayutthaya United', rep: 65 },
  utt: { id: 97208556, name: 'Uthai Thani FC', rep: 71 },
  lmp: { id: 23134767, name: 'Lamphun Warriors', rep: 67 },
  suk: { id: 23077994, name: 'Sukhothai FC', rep: 62 },
  mtn: { id: 23078012, name: 'Muangthong United', rep: 72 },
  nks: { id: 5637857, name: 'Nakhon Ratchasima', rep: 64 },
  kpw: { id: 97208564, name: 'Kanchanaburi Power', rep: 72 },
}
...
function resolveAndBuildPlayers() {
  ...
  fs.writeFileSync(
    IDS_OUT,
    JSON.stringify(
      { source: 'fmtransferupdate Thai League 1', matched: Object.keys(byName).length, byName },
      null,
      2,
    ),
  )
    ...
    JSON.stringify(
      {
        ...
        note: 'Not for redistribution or commercial sale. Thai League 1 / tha pack.',
        clubs,
      },
      null,
      2,
...
async function fetchFmInside() {
  ...
  let skip = 0
  let fail = 0
  const tmp = path.join(FMIN_DIR, '_tmp_tha.html')

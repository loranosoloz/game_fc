...
export function MatchPage() {
  ...
  const dayFixtures =
    next != null
      ? save.fixtures.filter((f) => f.matchday === next.matchday)
      : []
      ...
      <section className="rounded-xl border border-slate-200 bg-white/80 p-5">
        ...
        {next ? (
          <div className="mt-3 space-y-3">
            ...
            <p className="text-sm text-slate-600">
              Same day: <strong>{dayFixtures.length}</strong> fixtures across the league (including
              AI vs AI).
            </p>
            <button
              type="button"
          ...
          <p className="mt-3 text-sm text-slate-600">No remaining fixtures for your club.</p>
        )}
          ...
          <div className="mt-6 border-t border-slate-100 pt-4">
            ...
            <ul className="mt-2 space-y-1 text-sm text-slate-600">
              {last.events
                ...
                .map((e, i) => (
                  <li key={`${e.minute}-${i}`}>
                    {e.minute}&apos; {e.playerName} ({tag(e.clubId)})
                  </li>
                ))}
            </ul>
          </div>
